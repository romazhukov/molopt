# optimize/xtb_optimizer.py
from pathlib import Path
import subprocess, shutil
from io_mod.xyz_reader import write_xyz
from energy.xtb_parser import parse_total_energy

class XTBOptimizer:
    def __init__(self, level="gfn2", opt="tight", debug=False):
        self.level = level
        self.opt = opt
        self.debug = debug

    def _check_xtb(self):
        if not shutil.which("xtb"):
            raise RuntimeError("xtb не найден в PATH (проверь установку и PATH)")

    def optimize(self, atoms, coords, charge=0, multiplicity=1, workdir=".", index=1):
        """
        1) Пишем входной файл conf.xyz в results/xtb_XXX/
        2) Запускаем xTB: xtb conf.xyz --opt ... --chrg ... --uhf ... --gfn2/--gfn1/--gfn0
        3) Парсим энергию и xtbopt.xyz
        """
        self._check_xtb()

        uhf = max(0, int(multiplicity) - 1)
        wd = Path(workdir) / f"xtb_{index:03d}"
        wd.mkdir(parents=True, exist_ok=True)

        # 1) записываем вход
        inp = wd / "conf.xyz"
        write_xyz(inp, atoms, coords, comment=f"conf {index}")
        if not inp.exists() or inp.stat().st_size == 0:
            raise RuntimeError("Не удалось записать входной файл conf.xyz")

        # 2) команду формируем так, чтобы первый аргумент был ИМЕННО входной файл
        cmd = ["xtb", "conf.xyz", "--opt", self.opt, "--chrg", str(charge), "--uhf", str(uhf)]
        if self.level:
            cmd.append(f"--{self.level}")   # например --gfn2

        (wd / "cmd.txt").write_text(" ".join(cmd) + f"\nabs: {inp.resolve()}\n")

        # Запуск
        res = subprocess.run(cmd, cwd=wd, capture_output=True, text=True)
        (wd / "stdout.txt").write_text(res.stdout or "")
        (wd / "stderr.txt").write_text(res.stderr or "")

        # 3) парсим энергию (из лога или stdout)
        out_all = (res.stdout or "") + "\n" + (res.stderr or "")
        energy = None
        log = wd / "xtbopt.log"
        if log.exists():
            energy = parse_total_energy(log.read_text())
        if energy is None:
            energy = parse_total_energy(out_all)

        # заберём оптимизированную геометрию, если она есть
        atoms_opt, coords_opt = atoms, coords
        opt_xyz = wd / "xtbopt.xyz"
        if opt_xyz.exists():
            lines = opt_xyz.read_text().splitlines()
            try:
                n = int(lines[0].strip())
                A, C = [], []
                for line in lines[2:2+n]:
                    s = line.split()
                    A.append(s[0]); C.append((float(s[1]), float(s[2]), float(s[3])))
                atoms_opt, coords_opt = A, C
            except Exception:
                pass

        if self.debug:
            print(f"[DEBUG xtb] rc={res.returncode} dir={wd}")
            print("\n".join(out_all.splitlines()[:30]))

        meta = {
            "status": "ok" if energy is not None else "failed",
            "workdir": str(wd),
            "returncode": res.returncode,
            "cmd": " ".join(cmd),
        }
        return atoms_opt, coords_opt, energy, meta
